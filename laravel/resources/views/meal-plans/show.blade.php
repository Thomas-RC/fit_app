...
@section('content')
<div class="py-12" x-data="{ confirmDelete: false }">
        ...
        <div class="fit-card green-bg-stripes p-6">
            ...
            <div class="mb-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <form action="{{ route('meal-plans.destroy', $mealPlan) }}" method="POST" @submit.prevent="if(confirmDelete) { $el.submit(); } else { showToast('Kliknij ponownie aby potwierdzić usunięcie', 'error', 4000); confirmDelete = true; setTimeout(() => confirmDelete = false, 5000); }">
                        @csrf
                        @method('DELETE')
                        <button type="submit" :class="confirmDelete ? 'text-red-800' : 'text-red-600'" class="hover:text-red-700 flex items-center gap-2 text-sm font-semibold">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
                            <span x-show="!confirmDelete">Usuń plan</span>
                            <span x-show="confirmDelete">⚠️ Potwierdź usunięcie</span>
                        </button>
                    </form>
                </div>
            ...
            <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div class="text-sm text-blue-700">
                        <strong>Uwaga:</strong> Makroskładniki (białko, węglowodany, tłuszcze) są obliczane tylko dla składników z Twojej lodówki, które mają dane odżywcze z Spoonacular API. Składniki do dokupienia nie są uwzględnione w obliczeniach.<br><br>
                        <strong>Wskazówka:</strong> Zakup potrzebne produkty i dodaj je do lodówki, abyśmy mogli obliczyć dokładną wartość odżywczą wszystkich składników.
                    </div>
                </div>
            </div>
